...
program
  ...
  .option(
    "-t, --types <types>",
    "comma-separated list of cache types (package-manager,build-tool,browser,ide,system)",
  )
  .option("-e, --exclude <tools>", "comma-separated list of tools to exclude")
  .option(
    "--include <tools>",
    "comma-separated list of tools to include (overrides --all and --exclude)",
  ) // New option
  .option(
    "-d, --dry-run",
  ...
  .option(
    "--older-than <age>",
    "clean caches older than specified age (e.g., 7d, 2w, 1m)",
  )
  .option("--newer-than <age>", "clean caches newer than specified age")
  .option(
    "--larger-than <size>",
    "clean caches larger than specified size (e.g., 100MB, 1GB)",
  )
  .option("--smaller-than <size>", "clean caches smaller than specified size")
  .option(
    "--use-case <case>",
    "clean specific use cases (development,testing,production,experimental,archived)",
  )
  .option(
    "--priority <level>",
    "clean only specified priority (critical,important,normal,low)",
  )
...
program
  ...
  .action(async (options) => {
    try {
      ...
      printError(
        `Failed to list caches: ${error instanceof Error ? error.message : error}`,
      );
      process.exit(1);
    }
  });
...
program
  ...
  .action(async (options) => {
    try {
      ...
      printError(
        `Failed to show categories: ${error instanceof Error ? error.message : error}`,
      );
      process.exit(1);
    }
  });
...
program
  ...
  .action(async (options) => {
    try {
      ...
      printError(
        `Failed to get cache sizes: ${error instanceof Error ? error.message : error}`,
      );
      process.exit(1);
    }
  });
...
const configCmd = program
  ...
  .action(async (options) => {
    try {
      ...
      printError(
        `Failed to manage config: ${error instanceof Error ? error.message : error}`,
      );
      process.exit(1);
    }
  });
...
program
  ...
  .action(async (mode) => {
    try {
      ...
      printError(
        `Failed to set emoji mode: ${error instanceof Error ? error.message : error}`,
      );
      process.exit(1);
    }
  });
...
configCmd
  ...
  .option(
    "-i, --input <path>",
    "input config path (default: ~/.squeaky-clean/config.json)",
  )
  .option(
    "-o, --output <path>",
  ...
  .action(async (options) => {
    try {
      ...
      printError(
        `Failed to migrate config: ${error instanceof Error ? error.message : error}`,
      );
      process.exit(1);
    }
  });
...
program
  ...
  .action(async (options) => {
    try {
      ...
      printError(
        `Failed to run doctor: ${error instanceof Error ? error.message : error}`,
      );
      process.exit(1);
    }
  });
...
program
  ...
  .option(
    "-d, --dry-run",
    "show what would be thinned without actually thinning",
  )
  .option("-f, --force", "skip confirmation prompts")
  .action(async (options) => {
    try {
      ...
      printError(
        `Failed to thin binaries: ${error instanceof Error ? error.message : error}`,
      );
      process.exit(1);
    }
  });
...
program
  ...
  .action(async (options) => {
    try {
      ...
      printError(
        `Failed to run interactive mode: ${error instanceof Error ? error.message : error}`,
      );
      process.exit(1);
    }
  });
...
(async () => {
  ...
  if (
    ...
    (process.argv.slice(2).length === 1 && hasPristineFlag)
  ) {
