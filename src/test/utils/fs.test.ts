...
describe('File System Utilities', () => {
  ...
  describe('safeRmrf', () => {
    ...
    it('should delete directories recursively', async () => {
      vol.mkdirSync('/home/testuser/.cache/test-dir/subdir', { recursive: true });
      vol.writeFileSync('/home/testuser/.cache/test-dir/file.txt', 'content');
      vol.writeFileSync('/home/testuser/.cache/test-dir/subdir/nested.txt', 'nested');
    ...
    it('should reject unsafe paths outside home/cache directories', async () => {
      ...
      await expect(safeRmrf('/system/important/file.txt'))
        .rejects.toThrow('Refusing to delete path outside safe directories');
    });
    ...
    it('should allow deletion of cache directories', async () => {
      ...
      await expect(safeRmrf('/home/testuser/cache/file.txt'))
        .resolves.not.toThrow();
    });

    it('should allow deletion of node_modules', async () => {
      vol.mkdirSync('/home/testuser/project/node_modules', { recursive: true });
      vol.writeFileSync('/home/testuser/project/node_modules/package.json', '{}');
      
      await expect(safeRmrf('/home/testuser/project/node_modules'))
        .resolves.not.toThrow();
    });
    ...
    it('should allow deletion of temp directories', async () => {
      ...
      await expect(safeRmrf('/tmp/temp-file.txt'))
        .resolves.not.toThrow();
    });
  });
  ...
  describe('createDirectoryIfNotExists', () => {
    ...
    it('should not throw if directory already exists', async () => {
      ...
      await expect(createDirectoryIfNotExists('/test/existing'))
        .resolves.not.toThrow();
    });
  ...
  describe('getFileModificationTime', () => {
    ...
    it('should handle stat errors gracefully', async () => {
      const fs = await import('fs/promises');
      vi.spyOn(fs, 'stat').mockRejectedValueOnce(new Error('ENOENT: no such file'));
