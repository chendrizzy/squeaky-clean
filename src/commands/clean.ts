...
import { printInfo, printSuccess, printError, printWarning, printCleanComplete, symbols, formatSizeWithColor } from '../utils/cli';
  ...
  try {
    ...
    const types: CacheType[] | undefined = options.types ? 
      (Array.isArray(options.types) ? options.types.join(',') : options.types)
        .split(',').map(t => t.trim()) as CacheType[] : undefined;
    ...
    if (!options.dryRun && !options.force && config.shouldRequireConfirmation()) {
      ...
      printInfo('For now, running in dry-run mode to show what would be cleaned...');
      options.dryRun = true;
    }
    ...
    if (options.sizes && !options.dryRun) {
      ...
      try {
        ...
        sizeSpinner.succeed(`Total cache size found: ${formatSizeWithColor(totalSize)}`);
      } catch (error) {
    ...
    // Clean the caches
    const cleanSpinner = ora(options.dryRun ? 'Analyzing caches...' : 'Cleaning caches...').start();
    let results;
    try {
      ...
      cleanSpinner.fail(options.dryRun ? 'Failed to analyze caches' : 'Failed to clean caches');
      throw error;
    }
    ...
    for (const result of results) {
      if (result.success) {
        ...
        if (freed > 0) {
          ...
          printSuccess(`${result.name}: ${freedFormatted} ${options.dryRun ? 'would be' : ''} freed ${symbols.bubbles}`);
        } else {
    ...
    if (totalFreed > 0) {
      ...
      printSuccess(`Total ${options.dryRun ? 'potential ' : ''}space freed: ${totalFreedFormatted}`);
    }
    
    if (errorCount > 0) {
      printWarning(`${errorCount} cache${errorCount > 1 ? 's' : ''} had errors`);
    }
    
    if (successCount > 0 && !options.dryRun) {
      printCleanComplete('Your dev environment is now squeaky clean!');
    } else if (options.dryRun) {
      printInfo(`${symbols.soap} Run without --dry-run to actually clean these caches`);
    } else {
