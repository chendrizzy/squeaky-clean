import { printInfo, printSuccess, printError, formatSizeWithColor, symbols } from '../utils/cli';
  ...
  try {
    ...
    try {
      ...
      if (foundCaches > 0) {
        ...
        printSuccess(`📊 Total cache size: ${totalFormatted} across ${foundCaches} cache${foundCaches > 1 ? 's' : ''}`);
        printInfo(`💡 Use 'squeaky clean --dry-run' to see what can be cleaned`);
      } else {
        printInfo('No cache data found. This might mean caches are already clean or tools aren\'t installed.');
      }
