import { printSuccess, printInfo, printError, printWarning, symbols } from '../utils/cli';
...
export async function configCommand(options: ConfigOptions): Promise<void> {
  try {
    ...
    if (!options.list && !options.get && !options.set && !options.enable && !options.disable && !options.reset) {
...
async function showCurrentConfig(): Promise<void> {
  ...
  console.log(`  ${symbols.folder} Verbose: ${currentConfig.output?.verbose ? chalk.green('enabled') : chalk.gray('disabled')}`);
  console.log(`  ${symbols.folder} Colors: ${currentConfig.output?.useColors !== false ? chalk.green('enabled') : chalk.gray('disabled')}`);
  console.log();
  
  // Safety settings
  console.log(chalk.bold('🔒 Safety Settings:'));
  console.log(`  ${symbols.folder} Require Confirmation: ${config.shouldRequireConfirmation() ? chalk.yellow('yes') : chalk.green('no')}`);
  console.log();
...
async function resetConfiguration(): Promise<void> {
  ...
  printInfo('All tools will be enabled and settings will be restored to defaults.');
...
async function showConfigPath(): Promise<void> {
  ...
  try {
    ...
    if (exists) {
      ...
      console.log(chalk.yellow('   ⚠ Config file will be created when first used'));
    }
  } catch (error) {
...
async function interactiveConfigWizard(): Promise<void> {
  ...
  console.log(chalk.gray('This wizard will help you configure squeaky-clean settings.\n'));
  ...
  const outputAnswers = await inquirer.prompt([
    ...
    },
    {
      type: 'confirm',
      name: 'useColors',
      message: 'Enable colored output (recommended for terminals that support colors)?',
  ...
  const safetyAnswers = await inquirer.prompt([
    {
      ...
      message: 'Require confirmation before cleaning caches (recommended for safety)?',
  ...
  console.log(chalk.gray('Configure which cache cleaners should be enabled by category.\n'));
  ...
  for (const [type, cleaners] of Object.entries(groupedCleaners)) {
    ...
    cleaners.forEach(cleaner => {
      toolAnswers[cleaner.name] = categoryAnswers.enabledTools.includes(cleaner.name);
    });
  }
  ...
  console.log(`  ${symbols.folder} Verbose output: ${outputAnswers.verbose ? chalk.green('enabled') : chalk.gray('disabled')}`);
  console.log(`  ${symbols.folder} Colored output: ${outputAnswers.useColors ? chalk.green('enabled') : chalk.gray('disabled')}`);
  console.log(`  ${symbols.folder} Require confirmation: ${safetyAnswers.requireConfirmation ? chalk.yellow('yes') : chalk.green('no')}`);
  
  console.log('\n🔧 Enabled tools:');
  const enabledTools = Object.entries(toolAnswers).filter(([_, enabled]) => enabled).map(([name, _]) => name);
  const disabledTools = Object.entries(toolAnswers).filter(([_, enabled]) => !enabled).map(([name, _]) => name);
