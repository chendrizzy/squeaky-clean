import { printInfo, printSuccess, printError, printWarning, formatSizeWithColor, symbols } from '../utils/cli';
...
export async function autoCommand(options: AutoOptions): Promise<void> {
  try {
    ...
    const availableCaches = cacheInfos.filter(info => info.isInstalled && (info.size || 0) > 0);
    ...
    for (const cache of availableCaches) {
      ...
      const lastModified = cache.lastModified ? new Date(cache.lastModified) : null;
      ...
      else if (sizeInMB > minSizeBytes / (1024 * 1024) && isTemporaryCache(cache.name)) {
        recommendations.push({
          cache,
    ...
    if (recommendations.length === 0) {
      ...
      printInfo('Your caches are either small, recent, or contain important data.');
    ...
    console.log();
    printInfo(`💾 Total recommended for cleaning: ${formatSizeWithColor(totalRecommendedSize)}`);
    ...
    const mode = options.aggressive ? 'aggressive' : options.safe ? 'safe' : 'smart';
    console.log();
      ...
      try {
        ...
        for (const result of results) {
          ...
          if (result.success) {
            ...
            if (freed > 0) {
              printSuccess(`${symbols.bubbles} ${result.name}: ${formatSizeWithColor(freed)} freed`);
            }
          } else {
            errorCount++;
        ...
        if (actualFreed > 0) {
          printSuccess(`🎉 Auto-cleaning complete! Freed ${formatSizeWithColor(actualFreed)}`);
...
function isTemporaryCache(cacheName: string): boolean {
  const temporaryCaches = [
    ...
    'webpack', 'vite', 'turbo',    // Build tools
    'docker'                       // System tools
  ];
