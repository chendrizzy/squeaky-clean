...
export const interactiveEnhancedCommand = new Command('interactive')
  .alias('i')
  .description('interactively select and clean caches with advanced granular control')
    ...
    try {
      ...
      if (options.profile) {
        ...
        if (profile) {
          ...
          console.log(chalk.green(`✅ Using profile: ${profile.icon} ${profile.name}`));
        ...
        switch (protectedAction) {
          case 'add':
            const { newPaths } = await inquirer.prompt([
              {
                type: 'input',
                name: 'newPaths',
                message: 'Enter paths to protect (comma-separated, supports glob patterns):',
            ...
            if (newPaths.length > 0) {
              ...
              console.log(chalk.green(`✅ Added ${newPaths.length} protected path(s)`));
            }
            break;
          ...
          case 'remove':
            if (protectedPaths.length > 0) {
              ...
              protectedPaths = protectedPaths.filter(p => !pathsToRemove.includes(p));
              config.set({ ...currentConfig, protectedPaths });
              console.log(chalk.green(`✅ Removed ${pathsToRemove.length} protected path(s)`));
            }
            break;
        }
      }
      ...
      allCleaners.forEach(cleaner => {
        ...
        if (categories && categories.length > 0) {
          console.log(`      ${chalk.cyan(`${categories.length} categories available`)}`);
        }
      });
      ...
      switch (selectionMethod) {
        ...
        case 'type':
          const types: CacheType[] = ['package-manager', 'build-tool', 'browser', 'ide', 'system'];
          ...
          selectedCleaners = allCleaners.filter(c => selectedTypes.includes(c.type));
          break;
      ...
      for (const cleaner of selectedCleaners) {
        ...
        console.log(`${getCacheEmoji(cleaner.type)} Processing ${chalk.bold(cleaner.name)}...`);
        ...
        try {
          ...
          if (categoryIds && cleaner.clearByCategory) {
            ...
            result = await cleaner.clearByCategory(categoryIds, isDryRun, info, protectedPaths);
          } else {
            // Clear with criteria or all
            result = await cleaner.clear(isDryRun, criteria, info, protectedPaths);
          }
          
          if (result.success) {
            const savedSize = result.sizeBefore - (result.sizeAfter || 0);
            console.log(chalk.green(`   ✅ ${isDryRun ? 'Would save' : 'Saved'}: ${formatSizeWithColor(savedSize)}`));
            
            if (result.clearedCategories && result.clearedCategories.length > 0) {
              console.log(chalk.gray(`      Categories: ${result.clearedCategories.join(', ')}`));
            }
            
            if (protectedPaths.length > 0 && result.clearedPaths) {
              const skippedCount = info.paths.length - result.clearedPaths.length;
              if (skippedCount > 0) {
                console.log(chalk.yellow(`      Skipped ${skippedCount} protected path(s)`));
              }
            }
          } else {
      ...
      if (!isDryRun) {
        ...
        console.log(chalk.yellow('This was a dry run. Run without preview to actually clean caches.'));
      }
