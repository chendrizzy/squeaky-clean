  ...
  private async findProjectYarnCaches(): Promise<string[]> {
    ...
    for (const searchDir of searchDirs) {
      ...
      if (Date.now() - startTime > searchTimeout) {
        printVerbose('Project cache search timed out, skipping remaining directories');
        break;
      }
      ...
      if (await pathExists(searchDir)) {
        try {
          ...
          const projects = await this.findYarnProjectsRecursively(searchDir, 1, startTime, searchTimeout);
          projectCaches.push(...projects);
          
          // Limit total project caches found to prevent excessive scanning
          if (projectCaches.length > 50) {
            printVerbose('Found many project caches, limiting search to prevent performance issues');
            break;
          }
        } catch (error) {
    ...
    try {
      ...
      for (const entry of limitedEntries) {
        ...
        if (entry.isDirectory() && !entry.name.startsWith('.') && entry.name !== 'node_modules') {
          ...
          if (Date.now() - startTime < timeout) {
            const subCaches = await this.findYarnProjectsRecursively(fullPath, maxDepth - 1, startTime, timeout);
