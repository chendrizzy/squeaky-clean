...
class FlutterCleaner implements CleanerModule {
  ...
  private async findFlutterProjectBuildDirs(): Promise<string[]> {
    ...
    for (const searchDir of searchDirs) {
      if (await pathExists(searchDir)) {
        try {
          const projects = await this.findFlutterProjectsRecursively(searchDir, 3);
  ...
  private async findFlutterProjectsRecursively(dir: string, maxDepth: number): Promise<string[]> {
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.isDirectory() && !entry.name.startsWith('.') && entry.name !== 'build') {
          ...
          const flutterDir = path.join(fullPath, '.flutter-plugins-dependencies');
          ...
          if (await pathExists(pubspecFile)) {
            ...
            try {
              ...
              if (pubspecContent.includes('flutter:') || pubspecContent.includes('flutter_')) {
          ...
          const subBuildDirs = await this.findFlutterProjectsRecursively(fullPath, maxDepth - 1);
          ...
          if (cachePath.includes('.pub-cache')) {
            ...
            for (const subdir of selectiveDirs) {
              ...
              if (await pathExists(subdirPath)) {
                ...
                printVerbose(`${symbols.soap} Cleared pub-cache subdirectory: ${subdir}`);
              }
            }
          } else {
  ...
  private async runFlutterCleanInProjects(): Promise<void> {
    ...
    for (const buildDir of projectDirs) {
      ...
      } else if (buildDir.endsWith('/.dart_tool') || buildDir.endsWith('\\.dart_tool')) {
        projectRoots.add(path.dirname(buildDir));
      }
    }
    ...
    for (const projectRoot of projectRoots) {
      try {
        ...
        printVerbose(`${symbols.soap} Executed 'flutter clean' in ${projectRoot}`);
      } catch (error) {
