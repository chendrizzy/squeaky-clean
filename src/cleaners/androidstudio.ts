...
export class AndroidStudioCleaner implements CleanerModule {
  ...
  private async findAndroidProjectBuildDirs(): Promise<string[]> {
    ...
    for (const searchDir of searchDirs) {
      if (await pathExists(searchDir)) {
        try {
          const projects = await this.findAndroidProjectsRecursively(
            searchDir,
            2,
          );
  ...
  private async findAndroidProjectsRecursively(
    dir: string,
    maxDepth: number,
  ): Promise<string[]> {
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.isDirectory() && !entry.name.startsWith(".")) {
          ...
          if (
            (await pathExists(gradleFile)) ||
            (await pathExists(gradleKtsFile)) ||
            (await pathExists(androidManifest))
          ) {
          ...
          const subBuildDirs = await this.findAndroidProjectsRecursively(
            fullPath,
            maxDepth - 1,
          );
