...
class CocoaPodsCleaner implements CleanerModule {
  ...
  description = "CocoaPods caches (macOS only)";

  private getCachePaths(): string[] {
    const home = os.homedir();
    return [path.join(home, "Library", "Caches", "CocoaPods")];
  }
    ...
    const categories: CacheCategory[] = [
      {
        ...
        description: "Pod downloads and specs cache",
  ...
  private async clearCategories(
    ...
    if (dryRun) {
      printVerbose(
        `[DRY RUN] Would clear CocoaPods cache: ${pathsToClear.join(", ")}`,
      );
      return {
        name: this.name,
        success: true,
    ...
    for (const cachePath of pathsToClear) {
      ...
      try {
        if (await pathExists(cachePath)) {
          printVerbose(`Clearing CocoaPods cache at ${cachePath}`);
  ...
  async clear(
    ...
    if (!info.isInstalled) {
      return {
        name: this.name,
        success: false,
        error: "cocoapods is not installed or not supported on this platform",
        clearedPaths: [],
        sizeBefore: 0,
        sizeAfter: 0,
      };
...
export default new CocoaPodsCleaner();
