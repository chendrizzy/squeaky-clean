...
export class HomebrewEnhancedCleaner extends BaseCleaner {
  ...
  private getCachePaths(): string[] {
    ...
    try {
      ...
      // Portable Ruby cache
      paths.push(
        path.join(homeDir, "Library", "Caches", "Homebrew", "portable-ruby"),
      );

      // Downloads
      paths.push(
        path.join(homeDir, "Library", "Caches", "Homebrew", "downloads"),
      );
  ...
  async getCacheCategories(): Promise<CacheCategory[]> {
    ...
    const categoryMap: {
      [key: string]: {
        name: string;
        priority: CacheCategory["priority"];
        useCase: CacheCategory["useCase"];
      };
    } = {
      downloads: {
    ...
    for (const cachePath of paths) {
      ...
      try {
        ...
        categories.push({
          id: `homebrew-${path.basename(cachePath)}`
            .toLowerCase()
            .replace(/[^a-z0-9-]/g, "-"),
          name: categoryName,
  ...
  async clear(
    ...
    const result = await super.clear(
      dryRun,
      criteria,
      cacheInfo,
      protectedPaths,
