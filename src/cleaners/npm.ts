    ...
    for (const cachePath of allPaths) {
      if (await pathExists(cachePath)) {
        ...
        try {
          ...
          const isMainNpmCache = cachePath.includes('.npm') && !cachePath.includes('node_modules');
          ...
          printVerbose(`Found npm cache at ${cachePath} (${size} bytes${isMainNpmCache ? ' estimated' : ''})`);
        } catch (error) {
