...
export class TurboCleaner implements CleanerModule {
  ...
  private async findTurboCaches(): Promise<string[]> {
    ...
    const globalTurboCache = path.join(
      homeDir,
      "Library",
      "Caches",
      "turborepo",
    ...
    for (const searchDir of searchDirs) {
      if (await pathExists(searchDir)) {
        try {
          const projectCaches = await this.findTurboCachesRecursively(
            searchDir,
            3,
          );
  ...
  private async findTurboCachesRecursively(
    dir: string,
    maxDepth: number,
  ): Promise<string[]> {
    ...
    try {
      ...
      for (const entry of entries) {
        if (
          ...
          const subCaches = await this.findTurboCachesRecursively(
            fullPath,
            maxDepth - 1,
          );
  ...
  private async searchForTurboRepos(
    dir: string,
    maxDepth: number,
  ): Promise<boolean> {
