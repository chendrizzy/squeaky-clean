...
export class CacheManager {
  ...
  async cleanAllCaches(
    ...
    for (const cleaner of cleaners) {
      try {
        ...
        const cacheInfoForCleaner = allCacheInfos.find(
          (info) => info.name === cleaner.name,
        ); // Find corresponding info

        let result: ClearResult;
        if (
          options.subCachesToClear &&
          options.subCachesToClear.has(cleaner.name)
        ) {
          ...
          if (cleaner.clearByCategory) {
            ...
            printVerbose(
              `Warning: ${cleaner.name} does not support granular clearing by category. Clearing all.`,
            );
        ...
        if (result.success && !options.dryRun) {
          ...
          if (savedMB > 0) {
            printVerbose(
              `${symbols.bubbles} ${cleaner.name}: Freed ${savedMB.toFixed(1)} MB`,
            );
          }
        }
      } catch (error) {
  ...
  async getSummary(options?: {
    ...
    const totalSize = allCacheInfo.reduce(
      (sum, info) => sum + (info.size || 0),
      0,
    );
    const installedCleaners = allCacheInfo.filter(
