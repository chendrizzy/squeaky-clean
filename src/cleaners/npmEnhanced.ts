...
export class NpmEnhancedCleaner extends BaseCleaner {
  ...
  private getCachePaths(): string[] {
    ...
    const commonCachePaths = ["node_modules/.cache", ".npm-cache", "npm-cache"];
    ...
    for (const cachePath of allPaths) {
      if (await pathExists(cachePath)) {
        ...
        // Project-specific caches
          ...
          const buildToolCaches = await this.getBuildToolCaches(
            cachePath,
            projectName,
          );
        ...
        // General npm cache
          ...
          categories.push({
            ...
            projectPath: this.isProjectSpecific(cachePath)
  ...
  private async getNpmCacheSubCategories(
    npmCachePath: string,
  ): Promise<CacheCategory[]> {
    const categories: CacheCategory[] = [];
  ...
  private async getBuildToolCaches(
    cachePath: string,
    projectName: string,
  ): Promise<CacheCategory[]> {
    const categories: CacheCategory[] = [];
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.includes("webpack")) {
          ...
          priority = this.isRecentlyUsed(fullPath, 1)
            ? "critical"
            : "important";
  ...
  async clear(
    ...
    for (const category of filteredCategories) {
      ...
      const pathsToClearInCategory = category.paths.filter((cachePath) => {
        ...
        if (isProtected) {
          printVerbose(
            `Skipping protected path in category ${category.name}: ${cachePath}`,
          );
        }
        return !isProtected;
      });
      ...
      if (dryRun) {
        printVerbose(
          `[DRY RUN] Would clear ${category.name}: ${pathsToClearInCategory.join(", ")}`,
        );
        totalSizeAfter += category.size || 0; // No change in dry run
      } else {
        try {
          for (const path of pathsToClearInCategory) {
