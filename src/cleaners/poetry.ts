...
class PoetryCleaner implements CleanerModule {
  ...
  description = "Python Poetry cache directories";

  private getCachePaths(): string[] {
    const home = os.homedir();
    const paths = [path.join(home, ".cache", "pypoetry")];

    if (process.platform === "darwin") {
      paths.push(path.join(home, "Library", "Caches", "pypoetry"));
    }

    if (process.platform === "win32") {
      const localAppData =
        process.env.LOCALAPPDATA || path.join(home, "AppData", "Local");
      paths.push(path.join(localAppData, "pypoetry", "Cache"));
    }
    ...
    const categories: CacheCategory[] = [
      {
        id: "cache",
        name: "Poetry cache",
        description: "Cached Poetry packages and metadata",
  ...
  private async clearCategories(
    ...
    if (dryRun) {
      printVerbose(
        `[DRY RUN] Would clear poetry cache: ${pathsToClear.join(", ")}`,
      );
      return {
        name: this.name,
        success: true,
    ...
    for (const cachePath of pathsToClear) {
      ...
      try {
        if (await pathExists(cachePath)) {
          printVerbose(`Clearing poetry cache at ${cachePath}`);
...
export default new PoetryCleaner();
