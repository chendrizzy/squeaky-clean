...
class NugetCleaner implements CleanerModule {
  ...
  description = ".NET NuGet package caches";

  private getCachePaths(): { packages: string; http: string } {
    const home = os.homedir();
    const packages = path.join(home, ".nuget", "packages");

    let httpCache = path.join(home, ".nuget", "v3-cache");
    if (process.platform === "win32") {
      const localAppData =
        process.env.LOCALAPPDATA || path.join(home, "AppData", "Local");
      httpCache = path.join(localAppData, "NuGet", "v3-cache");
    }

    return { packages, http: httpCache };
  }
    ...
    const categories: CacheCategory[] = [
      {
        id: "packages",
        name: "NuGet packages",
        description: "Downloaded packages cache",
      ...
      },
      {
        id: "http-cache",
        name: "HTTP cache",
        description: "HTTP responses and metadata cache",
        paths: [paths.http],
        priority: "normal",
        useCase: "development",
      },
  ...
  private async clearCategories(
    ...
    for (const cachePath of pathsToClear) {
      ...
      try {
        if (await pathExists(cachePath)) {
          printVerbose(`Clearing NuGet cache at ${cachePath}`);
  ...
  async clear(
    ...
    if (!info.isInstalled) {
      return {
        name: this.name,
        success: false,
        error: "nuget/dotnet is not installed",
        clearedPaths: [],
        sizeBefore: 0,
        sizeAfter: 0,
      };
...
export default new NugetCleaner();
