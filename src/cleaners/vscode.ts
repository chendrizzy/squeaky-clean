...
export class VSCodeCleaner implements CleanerModule {
  ...
  type = "ide" as const;
  description =
    "VS Code extensions cache, workspace storage, logs, and temporary files";
    ...
    for (const { path: cachePath, description, category } of allPaths) {
      if (await pathExists(cachePath)) {
        ...
        try {
          ...
          printVerbose(
            `Found ${category}: ${cachePath} (${sizeInMB} MB${sizeType}) - ${description}`,
          );
        } catch (error) {
